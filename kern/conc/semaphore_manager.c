...
void create_semaphores_array(uint32 numOfSemaphores) {
	...
	semaphores = (struct Semaphore*) kmalloc(
			numOfSemaphores * sizeof(struct Semaphore));
	if (semaphores == NULL) {
		panic(
...
#if USE_KHEAP
		{
			semaphores = (struct Semaphore*) krealloc(semaphores,
					2 * MAX_SEMAPHORES);
			if (semaphores == NULL) {
...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue) {
	...
	ID = allocate_semaphore_object(&SEMA);
	if (ID == E_NO_SEMAPHORE) {
			return E_NO_SEMAPHORE;
	}
	else if (ID != E_NO_SEMAPHORE) {
...
void signalSemaphore(int ownerEnvID, char* semaphoreName) {
	...
	if (ID != E_SEMAPHORE_NOT_EXISTS) {
		semaphores[ID].value+=1;
		if (semaphores[ID].value <= 0) {
			struct Env* e = dequeue(&semaphores[ID].env_queue);
